...
def getMilestoneDays(revenues, milestones):
    ...
    res = [0] * len(milestones)

    aggRevenue = [0] * len(revenues)
    aggRevenue[0] = revenues[0]

    for i, val in enumerate(revenues[1:]):
        aggRevenue[i+1] = val + aggRevenue[i]

    for i, val in enumerate(milestones):
        res[i] = findIdx(val, aggRevenue) + 1
