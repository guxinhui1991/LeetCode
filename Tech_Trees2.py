...
def count_of_nodes(root, queries, s):
    ...
    startVal = [0 for _ in queries]
    startNode = [None for _ in queries]
    queryChar = [q[1] for q in queries]
    s = ' ' + s
    for idx, query in enumerate(queries):
        startVal[idx] = query[0]

        queue = [root]
        while queue:
            node = queue.pop()
            if node.val == startVal[idx]:
                startNode[idx] = node
                break
            for c in node.children:
                if c: queue.append(c)

    for i, node in enumerate(startNode):
        queue = [node]
        while(queue):
            curNode = queue.pop()
            if s[curNode.val] == queryChar[i]: res[i] = res[i] + 1
